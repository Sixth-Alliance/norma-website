...
/**
 * Inline cart icon that uses `currentColor` so it adapts to the surrounding text color.
 * Use Tailwind text color classes on the parent (for example `text-black dark:text-white`).
 */
const CartIcon: React.FC<CartIconProps> = ({ className = "", size = 24, ariaLabel = "Cart", filled = true }) => {
  // The path is taken from the existing shopping-cart-02.svg but we rely on currentColor.
  return (
    <svg
